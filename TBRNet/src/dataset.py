...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, config, data_flist, mask_flist, GT_flist, additional_mask=[], augment=True):
        ...
        self.subject_word = config.SUBJECT_WORD
    ...
    def data_augment(self, img, mask_GT, GT):
        ...
        p.flip_random(1)
        if self.subject_word=="SRD":
            width=random.randint(256,480) # SRD
        elif self.subject_word=="ISTD":
            width=random.randint(256,420) # ISTD
        else:
            width = random.randint(int(self.input_size_w/3), self.input_size_w)
