...
def predict(config, args):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(data_loader)):
            ...
            allim2 = np.zeros(( h, w, c, p, q))
            out_rgb = np.clip(out_[:3], 0, 1)
      
            allim2[0, 0, :] = out_rgb * 255
            allim2 = allim2.transpose(0, 3, 1, 4, 2)
            allim2 = allim2.reshape((h*p, w*q, c))
            middle_dir = os.path.join(args.out_dir, 'middle')
            os.makedirs(middle_dir, exist_ok=True) 
            save_image(middle_dir, allim2 , i, 1, filename=filename)
